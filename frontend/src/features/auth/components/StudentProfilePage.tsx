...
import {
  ...
  Briefcase, School, Award, Hash, BookOpen, Layers, IdCard
} from 'lucide-react';
...
const StudentProfilePage = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        toast.error(err.message || "Failed to load student profile");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4 bg-transparent font-sans">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-sm text-slate-500">Could not retrieve student profile data.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-transparent font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden">
      <StudentProfileHero profileData={profileData} />
      ...
      <div className="relative z-20 max-w-[1200px] mx-auto px-4 md:px-8 lg:px-10 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
          ...
          <div className="lg:col-span-8 space-y-7">
            <StudentProfileAcademic profileData={profileData} />
            <StudentProfileContact profileData={profileData} />
          </div>
...
const StudentProfileHero = ({ profileData }: { profileData: any }) => {
  const handleShare = () => {
                ...
                <p className="text-slate-500 text-xs md:text-sm font-semibold flex flex-wrap items-center gap-2 mt-1.5">
                  ...
                  {profileData.identificationNumber && (
                    <span className="px-2.5 py-0.5 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-extrabold uppercase tracking-widest border border-slate-200 flex items-center gap-1">
                      <Hash size={10} /> UID: {profileData.identificationNumber}
                    </span>
                  )}
                </p>
              </div>
...
const StudentProfileAcademic = ({ profileData }: { profileData: any }) => (
  <motion.div
  ...
  >
    ...
    <div className="p-6 md:p-8 space-y-4">
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        ...
        <span className="text-sm font-bold text-slate-900 truncate">{profileData.branch || '---'}</span>
      </div>
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        <span className="text-sm font-medium text-slate-500 shrink-0">Year</span>
        <span className="text-sm font-bold text-slate-900 truncate">{YEAR_MAPPING[String(profileData.year)] || String(profileData.year)}</span>
      </div>
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        <span className="text-sm font-medium text-slate-500 shrink-0">Roll No.</span>
        <span className="text-sm font-bold text-slate-900 truncate">{profileData.rollNo || '---'} {profileData.division ? `(Div ${profileData.division})` : ''}</span>
      </div>
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        <span className="text-sm font-medium text-slate-500 shrink-0">College ID</span>
        <span className="text-sm font-bold text-slate-900 truncate bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">{profileData.collegeId || '---'}</span>
      </div>
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        <span className="text-sm font-medium text-slate-500 shrink-0">Admission Year</span>
        <span className="text-sm font-bold text-slate-900 truncate">{profileData.admissionYear || '---'}</span>
      </div>
      <div className="flex items-center justify-between py-1 gap-3 min-w-0">
        <span className="text-sm font-medium text-slate-500 shrink-0">Graduation Year</span>
        <span className="text-sm font-bold text-slate-900 truncate">{profileData.graduationYear || '---'}</span>
      </div>
    </div>
  </motion.div>
);

const StudentProfileContact = ({ profileData }: { profileData: any }) => (
  <motion.div
    initial={{ opacity: 0, y: 12 }}
    animate={{ opacity: 1, y: 0 }}
